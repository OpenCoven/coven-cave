  ...
  return (
    <div key={variant} className="flex min-h-[176px] w-full max-w-95 flex-col">
      ...
      <button
        ...
        className="-mx-1.5 flex w-fit items-center gap-2 rounded-bui-control px-1.5 py-1
          transition-colors duration-100 hover:bg-bui-hover-2"
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill={working ? "var(--bui-ink-2)" : "var(--bui-ink-3)"}>
          <path d="M12 2l2.4 7.2L22 12l-7.6 2.8L12 22l-2.4-7.2L2 12l7.6-2.8z" />
        </svg>
        {working ? (
          <span
            className="bg-clip-text text-[length:var(--text-base)] font-medium whitespace-nowrap text-transparent [background-image:linear-gradient(90deg,_var(--bui-ink-3)_35%,_var(--bui-ink)_50%,_var(--bui-ink-3)_65%)]! [background-size:200%_100%]! [animation:bui-shimmer-text_1.4s_linear_infinite]!"
          >
            {v.active}
          </span>
        ) : (
          <span
            className="text-[length:var(--text-base)] font-medium whitespace-nowrap text-bui-ink-2 [animation:bui-fade-in_350ms_ease-out_both]!"
          >
            {v.done}
          </span>
        )}
      ...
      >
        <div className="overflow-hidden">
          <div className="relative mt-1 ml-[5px] pl-4">
            <span
              aria-hidden
              className="absolute left-[3px] w-px bg-bui-line"
            ...
            {v.query && (
              <div className="flex h-6 items-center gap-2 px-1.5" style={{ animation: expanded ? "bui-fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" : undefined }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--bui-ink-3)" strokeWidth="2" strokeLinecap="round" className="shrink-0">
                  <circle cx="11" cy="11" r="7" />
                  <path d="M21 21l-4.3-4.3" />
                </svg>
                <span className="text-[length:var(--text-base)] text-bui-ink-2">{v.query}</span>
              </div>
            )}
            {v.rows.slice(0, visible).map((row, i) => {
              const content = (
                ...
                {variant === "Steps" && (
                  i < visible - 1 || !working ? (
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--bui-ink-3)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
                      <path d="M20 6L9 17l-5-5" />
                    </svg>
                  ) : (
                    <span className="size-3 shrink-0 rounded-full border-[1.5px] border-bui-line-strong border-t-bui-ink-2 [animation:spin_700ms_linear_infinite]!" />
                  )
                )}
                <span className={`min-w-0 truncate text-[length:var(--text-base)] ${variant === "Reasoning" ? "whitespace-normal leading-relaxed text-bui-ink-2" : "font-medium text-bui-ink"} ${variant === "Search" ? "animated-underline" : ""}`}>
                  {row.primary}
                </span>
                {row.secondary && (
                  <span className={`shrink-0 text-[length:var(--text-sm)] text-bui-ink-3 ${row.mono ? "font-mono" : ""}`}>
                    {row.secondary}
                  </span>
                )}
                {row.add !== undefined && (
              ...
              if (variant === "Search") {
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className={`${rowClass} transition-colors duration-150 hover:bg-bui-hover`}
                    style={animation}
                  >
                    {content}
                  </a>
              ...
              if (variant === "Coding") {
                ...
                return (
                  <button
                    ...
                    className={`${rowClass} transition-colors duration-150 ${selected ? "bg-bui-inset" : "hover:bg-bui-hover"}`}
                    style={animation}
                  >
                    {content}
                  </button>
            ...
            {variant === "Search" && stage >= 3 && (
              <span className="text-[length:var(--text-sm)] text-bui-ink-3 [animation:bui-fade-in_300ms_ease-out_both]!">
                +7 more
              </span>
            )}
            </div>
