...
function matchingTasks(
  ...
  const normalizedWorktreePath =
    normalizeAbsoluteWorktreePath(worktreePath);
  return tasks
    ...
    .filter(
      (record) =>
        ...
        structuredRecords.filter(
          (candidate) => {
            const candidatePath = normalizeAbsoluteWorktreePath(candidate.path);
            return (
              candidatePath !== null &&
              candidatePath === normalizeAbsoluteWorktreePath(record.path)
            );
          },
        ).length === 1,
    )
