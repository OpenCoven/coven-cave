...
const BASELINES = {
  ...
  offScaleSpacingPx: 1662, // -2 against main's 1664: main banked all +11 of the live call transcript's off-scale spacing (cave-zr9dx, a411e3bcf3) as micro-marks. Nine of them are — 6px glyph-to-label gaps on the engine chips, reply target/row, fix row and controls, and 2px nudges on the turn stack, the spoken-word mark and reply-clear: the same family the Chart Room, Weaves, Review Deck and GitHub-composer handoffs bank, where --space-2 would push an icon off its word. The tenth and eleventh were not. `.voice-call-overlay__turn-text` carried `padding: 6px 10px`, and it is not a sub-20px pill: it is a max-width 90% bordered, rounded --text-sm bubble at line-height 1.5 — a container, so it belongs on the scale. Snapped to --space-2/--space-3 here (10px sits exactly between them, so the bubble's horizontal-greater-than-vertical shape decided it, not proximity). Banking a container to avoid a two-character fix is what turns this ratchet into a rubber stamp. //
