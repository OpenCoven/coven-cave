    ...
    <div className="w-full max-w-95 overflow-hidden rounded-bui-card bg-bui-surface shadow-bui-card">
      ...
      <div className="primitive-card-bar flex items-center justify-between border-b border-bui-line">
        <span className="flex items-baseline gap-2">
          <span className="font-mono text-[length:var(--text-sm)] font-medium text-bui-ink">churn.ts</span>
          <span className="text-[length:var(--text-sm)] text-bui-ink-3">TypeScript</span>
        </span>
        <button
      ...
      {/* code */}
      <pre className="min-h-[137px] bg-bui-inset px-3 py-2.5 font-mono text-[length:var(--text-sm)] leading-[1.7]">
        {LINES.slice(0, count).map((line, i) => (
          ...
          >
            ...
            <span className="pl-2.5 whitespace-pre">
              {line.map((tok, j) => (
                <span key={j} style={{ color: tok.c ? COLORS[tok.c] : "var(--bui-ink-2)" }}>
                  {tok.t}
                </span>
              ))}
              {i === count - 1 && !done && (
                <span className="ml-0.5 inline-block h-3 w-[3px] translate-y-0.5 rounded-full bg-bui-accent" />
              )}
            </span>
          </div>
        ))}
