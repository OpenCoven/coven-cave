...
| `Coven Pr.dc.html` | 75 | # Coven Cave code reading experience | The full PR reader the Coding Room's review rail links to as "Full PR view". Every pane is backed by an API that already exists (`/api/github/{checks,comments,commit,diff,review,merge,resolve-thread,rerun}`) and `code-session-pr-panel.tsx` already holds the check/thread hooks to extract. Scoped in `cave-l82dm`. |
