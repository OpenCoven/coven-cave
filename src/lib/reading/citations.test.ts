...
test("parseCitations reads a worktree file reference into a repo citation", () => {
  {
    const parsed = parseCitations(
      'Read it.[^1]\n\n[^1]: src/lib/reading/citations.ts#L12-L18 "The parser" — the footnote reader\n',
    );
    assert.equal(parsed.citations.length, 1);
    const [citation] = parsed.citations;
    assert.deepEqual(citation.file, { path: "src/lib/reading/citations.ts", lineStart: 12, lineEnd: 18 });
    ...
    assert.equal(presentation.context, "src/lib/reading/citations.ts · L12–18");
  }
