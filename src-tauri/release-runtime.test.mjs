...
test("Windows first launch paints progress and supports recovery while the sidecar starts", async () => {
  ...
  assert.match(
    launcher,
    /spawn_sidecar_startup\(\s*app\.handle\(\)\.clone\(\),\s*startup_control,\s*"sidecar-startup",\s*1,\s*\)\?;\s*spawn_sidecar_supervisor\(app\.handle\(\)\.clone\(\)\)/,
    "Windows must start post-ready supervision beside the startup owner",
  );
  assert.match(
    launcher,
    /spawn_sidecar_startup\(\s*app\.clone\(\),\s*Arc::clone\(control\.inner\(\)\),\s*"sidecar-recovery",\s*attempt,\s*\)/,
