...
import { TOAST_CLEARANCE_HEADER_SELECTOR, toastHeaderClearance } from "@/lib/toast-header-clearance";
...
export function InboxToastStack({ toasts, onDismiss, onExpire, onSnooze, onOpen }: Props) {
  ...
  useToastHeaderClearance(stackRef, toasts.length > 0);
  ...
  return (
    ...
    <div ref={stackRef} className="inbox-toast-stack">
      {visible.map((g) => {
        const t = g.lead;
...
function useToastHeaderClearance(stackRef: RefObject<HTMLDivElement | null>, active: boolean) {
  useEffect(() => {
    ...
    let frame = 0;
    // Re-observed only when the set of headers changes: observe() always
    ...
    const measure = () => {
      ...
      if (clearance === null) stack.style.removeProperty("--inbox-toast-clearance");
      else stack.style.setProperty("--inbox-toast-clearance", `${clearance}px`);
    };
    function schedule() {
