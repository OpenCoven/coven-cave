...
assert.match(
  src,
  /tiles\.map\(\(tile, i\) =>[\s\S]*<Separator[\s\S]*<Panel[\s\S]*minSize="10%"[\s\S]*renderGridTile\(tile\)/,
...
assert.match(src, /halfSplitGeometry/, "even splits consume integer host geometry");
assert.match(src, /splitPresentation/, "the host selects panes or tabs from its own width");
assert.match(src, /data-presentation=\{presentation\}/, "the presentation is exposed to pane CSS");
