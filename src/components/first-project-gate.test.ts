...
test("the gate keeps drafts through failures, blocks blank or busy submits, and surfaces retryable alerts", () => {
  ...
  assert.match(
    src,
    /else \{\s*setSubmitError\(result\.error\);\s*setSubmitErrorCode\(result\.code \?\? null\);\s*\}/,
    "the gate preserves addChatProject's actionable creation failure and its server code",
  );
