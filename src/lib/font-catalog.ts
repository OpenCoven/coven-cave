/**
 * Bundled font registry. Every entry corresponds to a vendored family in
 * src/app/fonts.ts (generated by scripts/vendor-google-fonts.mjs) whose
 * `.variable` class is spread onto <html> by the root layout, so each cssVar
 * resolves anywhere in the app. Unselected fonts cost nothing at runtime:
 * they're declared with `preload: false` and @font-face only downloads files
 * for families that rendered text actually uses.
 *
