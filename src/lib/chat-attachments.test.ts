...

{
  ...
  try {
    ...
    const typescript = await fileToAttachment(
      new File(["export const value: number = 1;"], "source.ts", { type: "video/mp2t" }),
    );
    assert.equal(typescript.mimeType, "text/typescript");
    assert.match(
      typescript.dataUrl ?? "",
      /^data:text\/typescript;base64,/,
      "ambiguous browser MIME detection cannot discard a TypeScript payload",
    );
    assert.ok(
      normalizeChatAttachments([typescript])[0]?.dataUrl,
      "the canonical TypeScript payload survives server normalization",
    );
  } finally {
