...
import { abortWarm, invalidateIfDefined } from "@/lib/surfaces/surface-warm-cache";
import type { SurfaceWarmupSurface } from "@/lib/surfaces/surface-warmup-registry";
...
export function useSurfaceWarmup(): void {
  useEffect(() => {
    ...
    const runNext = () => {
      ...
      void import("@/lib/surfaces/surface-warmup-registry").then(({ warmSurface }) => warmSurface(surface, runnable))
        .then((result) => {
