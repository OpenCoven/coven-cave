...
test("the dependency chart is the room's own overlay, never a board field", () => {
  // Dependencies are canonical on the card (Card.dependencies), not in a
  // bare overlay field — every link and cut writes the card so every surface
  // sees the same edges.  The legacy overlay survives only as a read-side
  // migration store.
