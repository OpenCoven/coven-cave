...
const BASELINES = {
  ...
  inlineTsxStyles: 209, // +1: Sessions surface rebuild ("Chat Session - Prototype.dc.html" handoff, cave-n3jg2) passes `--row-tint` per familiar; the tint is editable user data, so no token can express it. +1: the summoning rite's model stones (cave-3rz.3) pass `--stone-scale` from `contextWindowForModel`; that scale exists only at render time and differs per model. The rest of the inline-style budget is unchanged: remaining sites are still dynamic percentages, measurements, runtime colours, and layout coordinates carried into CSS custom properties rather than static presentation.
};
