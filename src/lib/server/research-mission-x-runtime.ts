...
export function xSourceLedgerRef(
  ...
  // Identity and the user's own note only. Every field here is a function of
  // the SAVED source — nothing that was fetched reaches it, not the body and
  // not the author record. The handle in the title is read back out of the
  // stored canonical URL rather than off the post, so a durable record still
  // holds no author display data the design excludes it from holding.
  const authorUsername = usernameFromCanonicalUrl(source.canonicalUrl);
  return {
...
export async function hydrateMissionXSources(
  ...
  for (const source of attached) {
    if (source.availability === "deleted") {
      ...
      sources.push(xSourceLedgerRef(source, "deleted"));
      continue;
    }
