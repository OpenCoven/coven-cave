...
import { buildCodexExecArgs, prepareCodexChatRouting } from "@/app/api/chat/send/codex-routing";
import {
...
import {
  ...
  turnRetryModel,
} from "@/app/api/chat/send/chat-send-models";
import {
...
import { chatSse, startChatSseHeartbeat } from "@/app/api/chat/send/chat-send-sse";
import { conversationCwd, daemonSessionCwd, resolveFamiliarWorkspace } from "@/app/api/chat/send/chat-send-runtime";
import { resolveOpenClawGatewayOutcome } from "@/app/api/chat/send/openclaw-gateway-outcome";
...
function openClawChatResponse(args: {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start: async (controller) => {
      ...
      let runBuffer: RunBufferHandle | null = openRunBuffer([
        args.body.runId,
        conversationId,
      ]);
      const pushProgress = (
      ...
      if (gatewayDispatch.kind === "accepted") {
        ...
        runBuffer?.addKeys([args.body.runId, conversationId]);
        runBuffer?.setHooks({
          attach: () => {
        ...
        try {
          ...
          const isFirstExchange = await withConversationLock(conversationId, async () => {
            ...
            // createConversationStub settled before this lock was queued. If a
            // DELETE won the lock afterward, the missing file is authoritative:
            // never recreate it from this late Gateway completion.
...
export async function legacyExecuteChatSend(req: Request): Promise<Response> {
  let body: SendBody;
  try {
