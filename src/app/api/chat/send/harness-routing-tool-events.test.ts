...
assert.doesNotMatch(
  streamEvents.slice(
    streamEvents.indexOf('kind: "done"'),
    streamEvents.indexOf('| { kind: "error"'),
  ),
  /cancelled/,
  "The frozen done StreamEvent vocabulary must not expose cancellation",
);
...
assert.match(
  ...
  "The final done event must forward captured usage and cost without changing the frozen vocabulary (CHAT-D12-02)",
);
