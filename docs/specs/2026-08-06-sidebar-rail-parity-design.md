...
## Follow-up: the same parity across collapse, not just across sections

**Landed:** 2026-08-14 · **Issue:** #4351

The header above was made identical across Home and Chat. It was still not
identical across *collapse*. Hovering the collapsed 56px rail floats the panel
open as an overlay (hover-peek), and every control moved:
...
Same cause, one level up: the rail centres square controls in 56px while the
panel left-aligns rows in 232px, and nothing said those two had to resolve to
the same icon column. On top of that the rail carried two controls the panel has
no counterpart for (a decorative brand mark, an account avatar whose only action
duplicated Settings), and the footer was a *row* in the panel against a *stack*
in the rail.

The fix extends the same idea from "one declaration" to "one geometry". Three
constants on `.shell-nav` — `--rail-pad`, `--rail-lead`, `--rail-control` —
solve for the same 28px icon centre in both states, and they are not independent:

```
--rail-pad === (56 - 2 * --space-2 - --rail-control) / 2
```

Everything else follows from reading them: rows, header controls and footer
buttons take `--rail-control` for height and `--rail-lead` for leading padding,
and the two bordered controls subtract their edge so their glyphs still land on
the column. Measured after: `dx` 0 for every control, footer `dy` −1.

Three lessons worth keeping:

- **A rail-only control is a parity bug in waiting.** Anything the panel does
  not also render displaces everything after it by its own height.
- **Media-query overrides re-split the two states silently.** A
  `max-height: 760px` block set per-selector row heights and gaps; every one of
  them reached the panel only, because the rail's `.shell-nav--rail` rules are
  more specific and win. At 1280×720 the states diverged again. Density belongs
  on the shared constant, not on the selectors that read it.
- **Measure after the entry animation.** The peek slides in from
  ...
  `tests/sidebar-rail-peek-parity.spec.ts` awaits `getAnimations()` instead of a
  wall-clock guess.

Still deliberately unequal: the Home/Chat section tabs. Two stacked tabs cannot
be as tall as one row of two, so content below them shifts ~25px on peek — under
one row's pitch, so the control under the cursor does not change.
