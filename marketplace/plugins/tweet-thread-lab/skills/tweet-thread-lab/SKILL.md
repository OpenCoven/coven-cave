...
5. Bind each materialized candidate ID and SHA-256 to its declared strategy in `strategies.json`. Keep harness, model, and context identifiers out of candidates and under private namespaced strategy extensions when needed. Starting from the absolute real path of the loaded `SKILL.md`, derive its installed `tweet-thread-lab` plugin root and set `tooling.json.validatorPath` to the absolute installed `tweet-thread-lab/bin/tweet-thread-validate.mjs` regular file. Record the loaded skill path and validator path; reject relative, missing, or differently installed paths.
...
8. Materialize one strict blinded judge scorecard per judged arm across all six rubric dimensions, bound only to trial ID, committed public-trial SHA-256, and arm token. Before identity reveal or stopping unlock, create and persist `blinded-scorecard-set-commitment.json`; it must bind the trial, public/reveal commitments, exact sorted arm-token-to-scorecard-SHA mapping, set SHA, timestamp, and secret-backed HMAC without candidate identity. Retain it in the run and require it when revealing and converting every byte-equivalent canonical scorecard.
...
- Write `brief.json`, `evidence.json`, `voice-profile.json`, `strategies.json`, `tooling.json`, `execution-log.jsonl`, candidate files, raw deterministic result files, blinded judge scorecards, `blinded-scorecard-set-commitment.json`, revealed canonical scorecards, `manifest.json`, and, after a human decision, `approval.json`.
...
- Verify the scorecard set commitment was materialized before reveal/stopping unlock, contains no candidate identity, matches every canonical blinded scorecard byte-for-byte through its SHA mapping, and remains retained in the run.
