...
pub(super) fn ensure_trusted_main_caller(webview: &Webview, capability: &str) -> Result<(), String> {
    if webview.label() != "main" {
        warn!("denied {} IPC from non-main webview: {}", capability, webview.label());
        return Err(format!("{capability} commands are only available to the main app webview"));
    }
    ...
    if TRUSTED_MAIN_ORIGINS.lock().contains(&origin) {
        ...
        Err(format!("{capability} commands are not available to this origin"))
    }
}
