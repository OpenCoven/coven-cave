...
assert.match(
  surface,
  /const CHAT_SECTION_ITEMS[^=]*= \[\s*\{\s*id:\s*"conversation",\s*label:\s*"Chats"\s*\},\s*\{\s*id:\s*"projects",\s*label:\s*"Projects"\s*\}[\s\S]*?<Tabs<FamiliarsScope>[\s\S]*?items=\{CHAT_SECTION_ITEMS\}/,
