...
Keep `readHealth` and `checkHealthEnvelope` behavior unchanged. Extend `contractExpectations()` and its tests to assert the fixture declares `authority.defaultMode === "off"` while retaining the same API/minimum/release version checks. Do not add a live discovery option to this health-only probe; the takeover harness performs isolated process startup and validates default v1 plus enforce-mode v2.
