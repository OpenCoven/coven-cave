 ...
 * every one of these, and the first six are exactly what that denylist let
 * past — it tested only the character after `\\` and the literal `\\?\UNC\`,
 * so every device-prefixed spelling below sailed through it:
 *
 ...
 *     \\?\C:\..\..\UNC\host\C$\…
 *
 * The remaining two are the only ones it did refuse:
 *
 *     \\host\C$\…                          \\?\UNC\host\C$\…
 *
