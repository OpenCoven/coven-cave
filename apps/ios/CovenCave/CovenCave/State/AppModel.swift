...
enum AppTab: String, CaseIterable, Sendable { case chats, tasks, terminal, settings }
...
struct TerminalFamiliarHandoff: Equatable {
    ...
    var chatDraft: String {
        let location = projectRoot ?? "No registered project root"
        return """
...
final class AppModel {
    ...
    func requestTerminalFamiliarHandoff(draft: String, projectRoot: String?) {
        let trimmed = draft.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        terminalFamiliarHandoff = TerminalFamiliarHandoff(
            draft: trimmed,
