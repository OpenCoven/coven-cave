...
assert.match(
  attachmentsLib,
  /const cap = mediaMime \? MAX_ATTACHMENT_MEDIA_BYTES : MAX_ATTACHMENT_IMAGE_BYTES;\s*\n\s*if \(file\.size > cap\) \{[\s\S]*?attachment\.truncated = true;/,
  "Oversized image and media attachments should be capped at capture time (per-kind cap) and marked like truncated text",
);
