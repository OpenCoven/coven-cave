...
assert.doesNotMatch(source, /foregroundProcessId|isExactCandidateForeground/, "foreground authority must be the exact main HWND, not any same-process window");
assert.match(source, /className -ne "Ghost"/);
...
assert.match(source, /\$owner -eq \$ProcessId -and \$className -ne "Ghost" -and/);
...
assert.match(source, /real click[\s\S]*native title-bar band[\s\S]*SetCursorPos/);
...
assert.match(physicalClickFunction, /Assert-MainWindowNotGhosted \$MainWindow[\s\S]*Focus-MainWindow \$MainWindow[\s\S]*Assert-ExactMainForeground \$MainWindow[\s\S]*SetCursorPos[\s\S]*WindowFromPoint[\s\S]*Assert-MainWindowNotGhosted \$MainWindow[\s\S]*Assert-ExactMainForeground \$MainWindow[\s\S]*mouse_event\(\$MOUSE_LEFTDOWN/);
