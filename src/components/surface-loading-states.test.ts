...
assert.match(
  workspace,
  /catch \{\s*\n\s*if \(isCurrent\(\)\) setSessionsError\(true\);[\s\S]*?finally \{\s*\n\s*if \(!baseSessionsApplied && isCurrent\(\)\) setLoadedSessionsScope\(requestedScopeKey\);/,
...
assert.match(
  workspace,
  /const requestedScopeKey = capturedActiveId === null \? "all" : `familiar:\$\{capturedActiveId\}`;[\s\S]{0,4000}setLoadedSessionsScope\(requestedScopeKey\)/,
