...
`,
  ...
  try {
    ...
    assert.ok(
      dataFrames.every((frame) => /^id: \d+$/m.test(frame)),
      "every published event belongs to the canonical resumable sequence",
    );
    const ids = dataFrames.map((frame) => Number(/^id: (\d+)$/m.exec(frame)?.[1]));
...
test("the route is a thin delegate to the extracted service", async () => {
  const { readFile } = await import("node:fs/promises");
  const route = await readFile(
    fileURLToPath(new URL("../../app/api/chat/send/route.ts", import.meta.url)),
    "utf8",
  );
  assert.match(route, /return executeChatSend\(req\)/);
