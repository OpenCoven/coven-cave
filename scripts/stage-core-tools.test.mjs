...
async function baselineFixture() {
  ...
  assert.equal(
    ...
    "https://github.com/OpenCoven/coven-code/releases/download/v0.6.1/coven-code-linux-x86_64.tar.gz",
  );
...
async function failClosedFixtureCases() {
...

  {
    ...
    await assert.rejects(stageFixture(fixture), /@opencoven\/cli version mismatch.*0\.2\.3.*0\.0\.52/);
    assert.equal(fixture.calls.downloads.length, 0);
  }

  {
    const fixture = await createFixture({ codeVersion: "0.5.0" });
    await assert.rejects(stageFixture(fixture), /@opencoven\/coven-code version mismatch.*0\.6\.1.*0\.5\.0/);
    assert.equal(fixture.calls.downloads.length, 0);
  }
  ...
  for (const [fixtureOptions, expectedMessage] of [
    ...
    [
      ...
      /@opencoven\/cli-linux-x64 version mismatch.*0\.2\.3.*0\.0\.52/,
    ],
  ]) {
...

  {
    ...
    fixture.deps.probeVersion = async (request) => {
      ...
      return request.tool === "coven" ? "coven 9.9.9" : "coven-code v0.6.1";
    };
    await assert.rejects(stageFixture(fixture), /Coven CLI version mismatch.*0\.2\.3.*9\.9\.9/);
...
async function maintenanceCases() {
...

  {
    ...
    fixture.deps.probeVersion = async (request) => {
      fixture.calls.probes.push(request);
      return request.tool === "coven" ? "coven 0.2.3" : "coven-code 0.5.0";
    };
    await assert.rejects(
      verifyCoreTools({
        toolsDir: fixture.dest,
      ...
      /Coven Code version mismatch.*0\.6\.1.*0\.5\.0/,
    );
    assert.equal(fixture.calls.probes.length, 2);
  }
...
async function exactVersionTokenCases() {
  {
    ...
    fixture.deps.probeVersion = async (request) => {
      fixture.calls.probes.push(request);
      return request.tool === "coven" ? "coven 0.2.3.999" : "coven-code 0.6.1";
    };
    await assert.rejects(
      stageFixture(fixture),
      /Coven CLI version mismatch.*0\.2\.3/,
...

  {
    ...
    fixture.deps.probeVersion = async (request) => {
      fixture.calls.probes.push(request);
      return request.tool === "coven"
        ? "coven 0.2.3 conflicting-runtime 9.9.9"
        : "coven-code 0.6.1";
    };
    await assert.rejects(
      verifyCoreTools({
      ...
      /expected exactly 0\.2\.3.*0\.2\.3, 9\.9\.9/,
  ...
  for (const output of [
    "coven 0.2.3-beta",
    "coven 0.2.3+fixture-build",
    "coven 0.2.3adjacent",
  ]) {
    const fixture = await createFixture();
    await stageFixture(fixture);
    fixture.deps.probeVersion = async (request) =>
      request.tool === "coven" ? output : "coven-code 0.6.1";
    await assert.rejects(
      verifyCoreTools({
        toolsDir: fixture.dest,
        platform: "linux",
...
async function publicationRollbackCase() {
  {
    ...
    const canonicalDest = await realpath(fixture.dest);
...

  {
    ...
    const publicationFs = {
      ...
      rename: async (source, destination) => {
        ...
        if (source === backupPath && destination === canonicalDest) throw rollbackError;
        return renamePath(source, destination);
      },
    };
    let caught;
...
async function destinationSafetyMatrixCases() {
  ...
  assert.deepEqual(
    ...
    {
      dest: path.join(canonicalRoot, "safe-output", "tools"),
      nodeModules: path.join(canonicalRoot, "inputs", "node_modules"),
      legalRoot: path.join(canonicalRoot, "legal-inputs", "legal"),
    },
  );
  ...
  assert.deepEqual(
    ...
    {
      dest: path.join(canonicalRoot, "repository", "src-tauri", "resources", "tools"),
      nodeModules: path.join(canonicalRoot, "repository", "node_modules"),
      legalRoot: path.join(canonicalRoot, "repository"),
    },
...
async function canonicalToolsAncestorSymlinkCases() {
...

  {
    ...
    assert.deepEqual(
      ...
      {
        dest: path.join(canonicalRoot, "repository", "generated", "resources", "tools"),
        nodeModules: path.join(canonicalRoot, "repository", "node_modules"),
        legalRoot: path.join(canonicalRoot, "repository"),
      },
...
async function repositoryLegalAssetsCase() {
  ...
  const pinnedLegalAssets = [
    ...
    },
    {
      ...
      attributes: "-text whitespace=cr-at-eol",
    },
    {
  ...
  for (const asset of pinnedLegalAssets) {
    ...
    assert.equal(
      attributeLines.has(`${attributePath} ${asset.attributes ?? "-text"}`),
      true,
      `${attributePath} must preserve its pinned Git attributes`,
    );
...
Corresponding source: https://github.com/OpenCoven/coven/tree/v0.2.3
...
https://github.com/OpenCoven/coven-code/tree/v0.6.1
`;
  assert.equal(
