...
export async function deleteProjectAndRevokeGrants(
  ...
  | { deleted: false; cleaned: null }
  | { deleted: true; cleaned: { grants: number; groupGrants: number; proposals: number } }
> {
  return withProjectAuthLock(async () => {
    const projects = await loadProjectsAlreadyAuthorized();
    if (!projects.some((project) => project.id === projectId)) {
      // Never orphan-clean a project that was never registered (and never
      // return a false "deleted" for one already fully removed by a prior,
      // completed call — see below for why that can never leave orphans).
      return { deleted: false, cleaned: null };
    }
