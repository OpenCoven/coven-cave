...
import { isStrictRfc3339 } from "./proposal-authority.ts";
...
export function normalizeProposalTerminal(row: unknown): ProposalTerminalReceipt | null {
  if (!isRecord(row) || !Number.isSafeInteger(row.id) || Number(row.id) < 1
    ...
    || !isStrictRfc3339(row.decided_at) || !(row.detail === null || typeof row.detail === "string")) return null;
  let detail: unknown;
  ...
  // Decisions without an opened window have no close detail. Their
  ...
  if (row.event_type === "proposal_approved") {
    ...
    if (detail.window_close === null) return receipt("applied", "applied");
  }
  ...
  if (row.event_type === "proposal_approved" && close.reason === "applied" && close.replay_hash_matched === true) {
    ...
    && (close.reason !== "evidence_diverged" || close.replay_hash_matched === false)
