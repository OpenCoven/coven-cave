...
case "$*" in
  *"/actions/runs"*)
    ...
    elif [ "\${LIFECYCLE_STRING_WORKFLOW_ID:-0}" = "1" ] &&
         [ "$WORKFLOW_STATE" = "queued" ]; then
      printf '%s\n' '[{"total_count":1,"workflow_runs":[{"id":"1006","status":"queued","head_branch":"feat/live","head_sha":"${workflowHead}","html_url":"https://example.test/run/1006"}]}]'
...
`,
  ...
  for (const [environment, expectedReason] of [
    ...
    ["LIFECYCLE_STRING_WORKFLOW_ID", /workflow inventory returned malformed data/],
