...
assert.match(
  src,
  /transcriptHandlersRef\.current = \{\s*siblingsFor,\s*switchBranch,\s*editTurnInComposer,\s*regenerateFor,\s*replyFor,\s*askAboutFor,\s*readerPromptFor,\s*rerunWithFor,\s*send,\s*approveAutoMission,\s*denyAutoMission,\s*sendApprovalAnswers,\s*approvalUnavailableReason,\s*\};/,
