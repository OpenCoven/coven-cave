...
assert.match(
  surface,
  /newChat\(\s*pendingChatAction\.projectRoot \?\? undefined,\s*pendingChatAction\.initialPrompt \?\? undefined,\s*pendingChatAction\.familiarId,\s*pendingChatAction\.origin,\s*pendingChatAction\.initialControls \?\? undefined,\s*pendingChatAction\.initialAttachments \?\? undefined,\s*\)/s,
  "ChatSurface should forward the pending origin, initial prompt, and attachments into ChatRouter.newChat",
);
