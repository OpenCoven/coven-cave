...
const BASELINES = {
  ...
  inlineTsxStyles: 230, // style={{…}} in TSX; many are legit dynamic values (banked from 500 — the count had drifted far below the old ceiling; -2: projects access page rebuild dropped the hub's inline styles; +1: vendored Blaze.tsx canvas host uses dynamic style spread for WebGL positioning; +2: project-setup modal color swatches paint runtime-only values (per-root tint + oklch palette choice))
};
