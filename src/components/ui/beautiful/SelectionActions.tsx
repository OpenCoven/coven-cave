...
import { Shimmer, StreamText } from "./atoms";
...
/* CAVE PORT: upstream draws these from `iconoir-react`. Cave has one icon
 * registry — the `ph:`-prefixed Phosphor union in src/lib/icon.tsx — and
 * adding a second icon package for ten glyphs would fork it, so each is mapped
 * to its nearest Phosphor equivalent. Both width AND height are passed: width
 * alone is a no-op, because height falls back to 1em and the glyph keeps its
 * inherited size (#1536). Phosphor glyphs carry their own weight, so
 * upstream's strokeWidth has no counterpart and is dropped. */
const iconProps = {
  width: 14,
  height: 14,
  "aria-hidden": true,
...
const icons = {
  explain: <Icon name="ph:chat-circle-dots" {...iconProps} />,
  improve: <Icon name="ph:sparkle" {...iconProps} />,
  shorten: <Icon name="ph:scissors" {...iconProps} />,
  tone: <Icon name="ph:mask-happy" {...iconProps} />,
  grammar: <Icon name="ph:file-text" {...iconProps} />,
  send: <Icon name="ph:arrow-up-bold" width={16} height={16} aria-hidden />,
  chevron: <Icon name="ph:caret-right" {...iconProps} />,
  check: <Icon name="ph:check" {...iconProps} />,
  close: <Icon name="ph:x" {...iconProps} />,
  retry: <Icon name="ph:arrows-clockwise" {...iconProps} />,
};

const control =
  "inline-flex h-7 shrink-0 items-center gap-1 rounded-full px-2.5 text-[length:var(--text-sm)] font-normal text-bui-ink transition-[background-color,color,transform] duration-150 hover:bg-bui-hover active:scale-[0.96]";

const primary =
  "inline-flex h-7 shrink-0 items-center gap-1 rounded-full bg-bui-ink px-2.5 text-[length:var(--text-base)] font-normal text-bui-canvas shadow-bui-hairline transition-[opacity,transform] duration-150 hover:opacity-90 active:scale-[0.96]";
  ...
  return (
    <div className="w-full max-w-[460px]">
      <div ref={hostRef} className="relative select-none pb-12">
        <p className="text-[length:var(--text-base)] leading-relaxed text-bui-ink">
          {LEAD}
          <span
            ref={selectionRef}
            className="box-decoration-clone rounded-[3px] bg-[color-mix(in_srgb,var(--bui-accent)_14%,var(--bui-surface))] text-bui-ink dark:bg-bui-accent-tint"
          >
        ...
        >
          ...
          <div
            ref={barRef}
            className="flex h-9 w-fit max-w-[calc(100vw-48px)] items-center justify-center gap-0.5 overflow-hidden rounded-full bg-bui-surface p-1 font-sans font-normal text-bui-ink antialiased shadow-bui-overlay"
            style={{
              width:
              ...
              ...(visible
                ? {
                    animation:
                      "bui-pop-in 220ms cubic-bezier(0.23,1,0.32,1) both",
                  }
                : {}),
            }}
          >
            ...
            >
            {busy && (
              <span className="inline-flex h-7 items-center gap-1.5 whitespace-nowrap px-2.5 text-[length:var(--text-base)] font-normal text-bui-ink-2">
                <span
                  className="size-3 shrink-0 rounded-full border-[1.5px] border-bui-line-strong border-t-bui-ink-2 [animation:spin_700ms_linear_infinite]!"
                />
                {mode === "thinking" ? (
                  <Shimmer className="text-[length:var(--text-base)] font-normal">
                    {busyLabel}…
                  </Shimmer>
                ) : (
                  <span>{busyLabel}…</span>
            ...
            {mode === "result" && (
              <>
                ...
                <span className="mx-0.5 h-4 w-px shrink-0 bg-bui-line" />
                <button
                  type="button"
                  aria-label="Try again"
                  onClick={() => run(action)}
                  className="flex size-7 shrink-0 items-center justify-center rounded-full text-bui-ink-3 transition-[background-color,color,transform] duration-150 hover:bg-bui-hover-2 hover:text-bui-ink-2 active:scale-[0.96]"
                >
                  {icons.retry}
                </button>
              </>
            ...
            {mode === "idle" && (
              <>
                ...
                >
                  ...
                  >
                    <input
                      ...
                      className="h-7 w-full bg-transparent pr-2.5 pl-3 text-[length:var(--text-base)] text-bui-ink placeholder:text-bui-ink-3"
                    />
                  </form>
                </div>
                ...
                >
                  {!expanded && (
                    <span className="mx-1 h-4 w-px shrink-0 bg-bui-line-strong" />
                  )}
                  ...
                  <button
                    ...
                    className="flex size-7 shrink-0 items-center justify-center rounded-full text-bui-ink transition-[background-color,transform] duration-200 hover:bg-bui-hover active:scale-[0.96]"
                  >
                    <span
                ...
                >
                  <button
                    ...
                    className="flex size-7 shrink-0 items-center justify-center rounded-full bg-bui-ink text-bui-surface transition-[opacity,transform] duration-200 active:scale-[0.94]"
                  >
                    {icons.send}
                  </button>
                </div>
