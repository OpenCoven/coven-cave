...
assert.doesNotMatch(src, /reveal-on-hover|reveal-scope/, "all three Code controls stay visible without hover");
assert.match(
  src,
  /focus-ring\$\{pinned \? " is-on" : ""\}/,
  "pin stays visible with a pressed state",
);
assert.match(
  src,
  /focus-ring\$\{isFullscreen \? " is-on" : ""\}/,
  "fullscreen stays visible",
);
assert.doesNotMatch(
  src,
