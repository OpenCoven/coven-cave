...
/* CAVE PORT: upstream fires a full-spectrum WebGL rainbow across the composer
 * interior when the flagship model is selected, via the `glimm` package. That
 * sweep is purely decorative, so it is dropped here rather than adding a
 * dependency — `celebrate()` below keeps the trigger point, and the accent
 * pulse it now runs stays on the Cave palette instead of introducing seven
 * hues the design contract does not define. */
 ...
 *
 * `no-render-hex-color` is disabled for exactly this block, and only this
 * block. The rule asks for a semantic token instead of a literal, which is
 * the right demand for every other colour in this directory — but these are
 * third-party BRAND marks. Slack's aubergine and Figma's five hues are fixed
 * by their owners' brand guidelines; they do not vary with our palette or
 * mode, and there is no Cave token that could correctly stand in for them.
 * Tokenizing them would either recolour someone else's logo or mint eleven
 * meaningless tokens. Hoisting the literals into consts to slip past the
 * rule would be worse still — evasion rather than a decision on the record.
 *
 * Scope discipline: the disable ends with the closing brace below, so a new
 * colour added anywhere else in this file is still caught. */
/* eslint-disable coven-design/no-render-hex-color */
...
/* eslint-enable coven-design/no-render-hex-color */
...
export function PromptBar({ variant = "Rounded" }: { variant?: string }) {
  ...
  /* CAVE PORT: upstream plays a WebGL rainbow sweep here (see the note at the
   * top of the file). The trigger is kept — selecting the flagship model still
   * marks the moment — but it now runs a one-shot accent pulse on the composer
   * shell, which needs no shader, no canvas, and no second palette.
   *
   * The reduced-motion bail-out is upstream's and is deliberately retained:
   * this is a celebratory flourish with no informational content, so the
   * correct reduced-motion story is not to play it at all. */
  const celebrate = () => {
    ...
    const done = () => {
      shell.classList.remove("bui-prompt-bar--celebrating");
      sweepingRef.current = false;
    };
    shell.addEventListener("animationend", done, { once: true });
    shell.classList.add("bui-prompt-bar--celebrating");
  };
    ...
    >
      ...
      {menu && (
        <div
          ...
          className="absolute inset-x-0 bottom-full z-10 mb-2 rounded-[10px] bg-bui-surface p-1 shadow-bui-raised [animation:bui-pop-in_180ms_cubic-bezier(0.23,1,0.32,1)_both]! [transform-origin:bottom_center]!"
        >
          {/* single gliding highlight — appears once a row is hovered */}
          <span
            aria-hidden
            className="pointer-events-none absolute inset-x-1 rounded-[6px] bg-bui-hover"
            style={{
              top: rowBox?.top ?? 0,
          ...
          {rows.map((row, i) => {
            ...
            return (
              ...
              >
                {source && (
                  <span className="flex size-5.5 shrink-0 items-center justify-center text-bui-ink-2">
                    {source.brand ? BRANDS[source.brand] : <Icon size={15}>{GLYPHS[source.glyph ?? "clip"]}</Icon>}
                  </span>
                )}
                <span className="shrink-0 text-[length:var(--text-base)] font-medium text-bui-ink">
                  {row.name}
                </span>
                <span className="min-w-0 flex-1 truncate text-[length:var(--text-sm)] text-bui-ink-3">{row.desc}</span>
                {source?.connect && (
                  <span
                    role="button"
                    tabIndex={-1}
          ...
          {rows.length === 0 && (
            <div className="flex h-9 items-center px-2 text-[length:var(--text-sm)] text-bui-ink-3">
              No matches for “{query}”
            </div>
          )}
          <div className="mt-1 border-t border-bui-line px-2 pt-1.5 pb-1 text-[length:var(--text-xs)] text-bui-ink-3">
      ...
      {modelOpen && (
        <div
          ...
          className="absolute right-0 bottom-full z-10 mb-2 w-44 rounded-[10px] bg-bui-surface p-1 shadow-bui-raised [animation:bui-pop-in_180ms_cubic-bezier(0.23,1,0.32,1)_both]! [transform-origin:bottom_right]!"
        >
          ...
          {MODELS.map((m, i) => (
            ...
            >
              <span className="min-w-0 flex-1 truncate text-[length:var(--text-base)] font-medium text-bui-ink">{m.name}</span>
              <span className="shrink-0 text-[length:var(--text-xs)] text-bui-ink-3">{m.tag}</span>
              <span className={`shrink-0 text-bui-ink ${m.key === model.key ? "" : "invisible"}`}>
      ...
      <div
        ref={shellRef}
        className={`bui-prompt-bar relative isolate flex flex-col gap-1.5 overflow-hidden border border-bui-line bg-bui-surface p-1.5 shadow-bui-card transition-[border-color,border-radius] duration-150 focus-within:border-bui-line-strong ${
      ...
      >
        <span
          ref={measureRef}
          aria-hidden="true"
          className="pointer-events-none absolute invisible whitespace-pre text-[length:var(--text-base)] leading-[18px]"
        >
          {draft}
        </span>
        ...
        {attachments.length > 0 && (
          <div className={`flex flex-wrap gap-1.5 pt-0.5 ${pill ? "px-1" : "px-0.5"}`}>
            {attachments.map((file, i) => (
              <span
                key={`${file}-${i}`}
                className={[(`flex h-6.5 items-center gap-1.5 bg-bui-field py-1 pr-1 pl-1.5 text-[length:var(--text-sm)] text-bui-ink-2 shadow-bui-hairline ${
                  pill ? "rounded-full" : "rounded-bui-chip"
                }`), "[animation:bui-pop-in_200ms_cubic-bezier(0.23,1,0.32,1)_both]!"].filter(Boolean).join(" ")}
              >
                ...
                <button
                  ...
                  className={`flex size-4 items-center justify-center text-bui-ink-3 transition-colors duration-100 hover:bg-bui-line/70 hover:text-bui-ink ${
        ...
        >
          <button
            ...
            className={`flex size-7 shrink-0 items-center justify-center justify-self-start text-bui-ink-3 transition-[background-color,color,transform] duration-150 hover:bg-bui-hover hover:text-bui-ink active:scale-[0.94] ${
              pill ? "rounded-full" : "rounded-[var(--radius-control)]"
            } ${plusOpen ? "bg-bui-hover text-bui-ink" : ""} ${expanded ? "col-start-1 row-start-2" : "col-start-1 row-start-1"}`}
          >
          ...
          <textarea
            ...
            aria-label="Prompt"
            className={`min-h-7 min-w-0 w-full resize-none bg-transparent px-1 py-[5px] text-[length:var(--text-base)] leading-[18px] text-bui-ink outline-none [overflow-wrap:anywhere] placeholder:text-bui-ink-3 ${
          ...
          <button
            ...
            className={`flex h-7 shrink-0 items-center gap-1 px-1.5 text-[length:var(--text-sm)] font-medium text-bui-ink-2 transition-colors duration-150 hover:bg-bui-hover hover:text-bui-ink ${
              pill ? "rounded-full" : "rounded-[var(--radius-control)]"
            } ${expanded ? "col-start-2 row-start-2" : "col-start-3 row-start-1"}`}
          >
            {model.name}
            <span className="text-bui-ink-3">
          ...
          <button
            ...
            className={`flex size-7 shrink-0 items-center justify-center transition-[background-color,color,transform] duration-150 active:scale-[0.94] ${
              pill ? "rounded-full" : "rounded-[var(--radius-control)]"
            } ${listening ? "bg-bui-accent-tint text-bui-accent-ink" : "text-bui-ink-3 hover:bg-bui-hover hover:text-bui-ink"} ${
              ...
              <span className="flex h-3.5 items-center gap-[2.5px]">
                {[0, 1, 2].map((i) => (
                  <span
                    ...
                    style={{ height: "100%", animation: `bui-eq-bounce 900ms ease-in-out ${i * 150}ms infinite` }}
                  />
                ))}
              </span>
            ) : (
