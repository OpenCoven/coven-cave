...
assert.match(src, /<ArtifactSourceView[\s\S]{0,200}?code=\{code\}[\s\S]{0,100}?kind=\{kind\}/, "the Code tab uses the shared highlighted source view");
...
assert.match(src, /showNavigationWarning\(\)/, "unexpected preview navigation shows the transient recovery warning");
assert.match(
  src,
  /\{runtimeError \? \([\s\S]{0,200}?role="alert"/,
  "runtime warnings retain assertive accessible announcement semantics",
);
