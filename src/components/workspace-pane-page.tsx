...
> {
  ...
  static getDerivedStateFromProps(
    ...
    if (props.resetKey !== state.resetKey) {
      return { errorMessage: null, retryKey: 0, resetKey: props.resetKey };
    }
    return null;
  }
  ...
  componentDidUpdate(
    _previousProps: WorkspacePaneErrorBoundaryProps,
    previousState: WorkspacePaneErrorBoundaryState,
  ) {
    if (this.state.errorMessage) {
      if (
        !previousState.errorMessage ||
        this.state.retryKey !== previousState.retryKey ||
        this.state.resetKey !== previousState.resetKey
      ) {
        this.focusRetry();
      }
      return;
    }
    if (previousState.errorMessage) this.focusPane();
  }
  ...
  private handleRetry = () => {
    this.setState((state) => ({ errorMessage: null, retryKey: state.retryKey + 1 }));
  };
      ...
      return (
        <div className="workspace-pane-page__state workspace-pane-page__state--error" role="alert">
          <div className="workspace-pane-page__state-copy">
            <p className="workspace-pane-page__state-title">Couldn't load {this.props.landmark}</p>
