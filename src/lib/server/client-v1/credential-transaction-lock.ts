// Crash-safe cross-process mutual exclusion for the client-v1 credential
// store, built on Node 24's built-in `node:sqlite` rather than a bespoke
// lock protocol of this module's own.
//
// The credential store's previous cross-process lock (`credential-file-lock.ts`,
// now deleted) serialized concurrent writers with a lock file created at a
// fixed path via `open(path, "wx")`, plus a "steal it from a dead owner"
// reclaim path that inspected the lock file's recorded owner and, if that
// owner looked dead, renamed a freshly created successor lock into place.
// That check-then-rename sequence is inherently racy: two reclaimers can
// both decide the same owner is dead and both rename a successor lock into
...
// ownership), the fixed lock file could be left in a state no later caller
...
// back for any decision) is never the source of truth for anything. The
// credential JSON file `credential-store.ts` reads and writes remains the
// sole persisted store; this module only ever runs the caller's
// read-modify-write `operation` while a `BEGIN IMMEDIATE` transaction on the
// adjacent lock database stays open, so it is provably true — not just
// documented — that at most one process is inside that critical section at
// a time.
...
export type CredentialTransactionLockOptions = {
  /** The resolved credential JSON store path this call is guarding. */
  storePath: string;
...
/**
 * Derive a credential store's adjacent SQLite lock database path. Exported
 * so tests and cleanup code can find (or remove) the sidecar lock file(s)
 * without duplicating this rule.
 */
export function credentialLockDbPath(storePath: string): string {
  return `${storePath}.lock.sqlite3`;
}

// Test-only seam: lets a test point a specific call at a different lock db
// path than `credentialLockDbPath` would derive, without ever touching the
// derivation `credentialLockDbPath` itself performs in production. Never
// set outside tests; production code never calls the setter, so the
// derivation above is exactly what every real caller gets.
...
export function setCredentialLockDbPathForTest(
...
function resolveLockDbPath(storePath: string): string {
  return (lockDbPathResolverForTest ?? credentialLockDbPath)(storePath);
}

// Lazily imported so every consumer of this module that never actually
// acquires the lock (most of `credential-store.ts`'s own test suite runs
// read-only paths) doesn't pay for loading `node:sqlite` at all, and so a
...
async function ensureLockDbDirectory(lockDbPath: string): Promise<void> {
  ...
  if (info.isSymbolicLink()) {
    throw new Error(`credential lock directory must not be a symlink: ${dir}`);
  }
}
  ...
  for (;;) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`credential lock: ${describe} timed out waiting for contention to clear`, { cause: error });
      }
 ...
 * never any credential, token, or store content, none of which this module
 * ever sees (it only ever holds a SQLite transaction open around the
 * caller's opaque `operation`). */
function logCleanupFailure(label: string, phase: string, error: unknown): void {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`[credential-transaction-lock] ${label}: ${phase} failed after lock acquisition (${message})`);
}

/**
 * Runs `operation` as the sole occupant of the credential store's
 ...
 *
 * `operation` must do its own read → mutate → write of the credential JSON
 * file: the transaction (and therefore the exclusion) stays open for its
 * entire duration, and the lock's own `COMMIT` is only attempted after
 ...
 */
export async function withCredentialTransactionLock<T>(
  options: CredentialTransactionLockOptions,
  operation: () => Promise<T>,
): Promise<T> {
  const timeoutMs = options.timeoutMs ?? DEFAULT_TIMEOUT_MS;
  const label = options.label ?? "credential-store";
  ...
  try {
    ...
    try {
      ...
      // wrote the credential store) before this lock's own commit failed.
    }
    return result;
  } finally {
    if (transactionOpen) {
