...
assert.match(
  chatViewSource,
  /const turnTools = turn\.tools \?\? \[\];[\s\S]*const indexedTurnTools = turnTools\.map\(\(tool, originalIndex\)[\s\S]*const editToolIds = new Set\([\s\S]*normalizeFileMutation\(tool\.name, tool\.input\)[\s\S]*const editCards = indexedTurnTools[\s\S]*const otherTools = indexedTurnTools[\s\S]*originalIndex/,
