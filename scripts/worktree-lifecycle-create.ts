...
function parseFullRecord(
  ...
  return {
    raw: value,
    branch,
    path: path.normalize(recordPath),
...
function execute(
  ...
  const unknownCreation: CreationEvidence = {
    path: worktreePath,
    ref: fullRef,
    oid: "unknown",
  };
  ...
  parseCoven(initialBead.metadata, root);
  ...
  if (
    ...
    const message = [
      ...
      .join("; ");
    if (recovered === null) {
      return partialOutcome(message, state, unknownCreation);
    }
    return compensate(
      root,
      options.branch,
