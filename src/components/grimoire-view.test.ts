...
assert.match(
  pageRegistry,
  /grimoire: \{[\s\S]{0,120}title: "Memories"/,
  "grimoire has a registry-backed page title (sr-only h1) reading Memories",
);
assert.match(
  workspace,
  /mode === "grimoire" \? \(\s*<GrimoireView\s+view=\{variant === "journal" \? "journal" : grimoireView\}/,
  "grimoire mode renders GrimoireView with the page variant or controlled view",
);
