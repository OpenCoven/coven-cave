...
> OpenCoven adaptation (2026-08-27): added secret-handling and remote
> state-change guardrails. The bundled reference documentation is unmodified
> from Daytona's upstream skill.
...
1. **SDK installed?** Check that the Daytona SDK is installed for the user's language (e.g. `pip show daytona` or check `package.json` for `@daytona/sdk`). Install it only when the user's task authorizes dependency changes.
2. **API key set?** Check only whether `DAYTONA_API_KEY` is present. Never print its value or read unrelated environment files to find it. If it is absent, tell the user they need an API key and point them to [Daytona Dashboard > API Keys](https://app.daytona.io/dashboard/keys) to create one.
3. **Remote change authorized?** Creating, starting, stopping, archiving, or modifying a sandbox changes remote state and may incur usage. Proceed only when the task explicitly authorizes that operation. Require specific confirmation before deleting a sandbox, snapshot, or volume.

Treat API keys, preview tokens, signed preview URLs, SSH access tokens, and
secrets placed inside sandboxes as sensitive. Keep them out of logs, source
files, command output, and final responses.
