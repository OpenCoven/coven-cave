...
### Features
- **Voice: the desktop app can hear you — native on-device recognition** — the packaged macOS app's WebView has no speech recognition engine, which left the keyless voice modes (Local, Familiar brain, ElevenLabs) deaf outside Chromium browsers. The speech loop's ears are now pluggable like its mouth: where Web Speech is missing, the desktop shell records each utterance (a small energy-based voice-activity gate segments the mic) and transcribes it through Apple's Speech framework via the new `speech_stt_probe`/`speech_stt_transcribe` Tauri commands — strictly on-device (`requiresOnDeviceRecognition`), so audio never leaves the Mac. First call prompts the system Speech Recognition permission; unusable states (denied, no on-device model for the language) fail at connect with actionable hints. Bundles gain microphone + speech-recognition usage descriptions and the hardened-runtime audio-input entitlement so packaged voice calls can open the mic at all (cave-0ogg).
