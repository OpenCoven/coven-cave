...
function createDeviceAccessGateway(options) {
  ...
  async function handle2(req, res) {
    ...
    try {
      ...
      if (pathname.startsWith("/api/client/") || pathname.startsWith("/api/pty") || pathname.startsWith("/api/passkey/register")) {
...
function assertStandaloneWindowsExclusive(path4, label) {
  ...
  if (findings.length > 0) {
    throw new Error(sharedOwnershipRefusal2(subject, path4, findings, waiver));
  }
  if (report.repaired) {
    console.warn(
...
function assertStandaloneDiscoveryTarget(path4) {
  try {
    ...
    if (!metadata.isFile() || metadata.isSymbolicLink()) {
      throw new Error(`Client v1 discovery target must be a regular file: ${path4}.`);
    }
