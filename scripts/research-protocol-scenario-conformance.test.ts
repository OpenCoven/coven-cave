...
test("binds every scenario input role to its exact protocol schema", () => {
  const expectedSchemas = new Map<string, ResearchProtocolObjectV1["schema"]>([
    ["run-manifest-revision:previous", "opencoven.run-manifest/v1"],
    ["run-manifest-revision:next", "opencoven.run-manifest/v1"],
    ["research-run-context-pack:run", "opencoven.research-run/v1"],
    ["research-run-context-pack:contextPack", "opencoven.context-pack/v1"],
    ["manifest-retention-consent:manifest", "opencoven.run-manifest/v1"],
    ["model-task-result:task", "opencoven.model-task/v1"],
    ["model-task-result:result", "opencoven.model-task-result/v1"],
    ["run-event-deletion:run", "opencoven.research-run/v1"],
    ["run-event-deletion:events[]", "opencoven.run-event/v1"],
    ["run-event-sequence:events[]", "opencoven.run-event/v1"],
  ]);
  ...
  for (const corpus of corpora) {
    for (const scenario of corpus.scenarios) {
      for (const entry of scenarioInputEntries(
        ...
        const role = entry.name.replace(/^events\[\d+\]$/, "events[]");
        const key = `${scenario.validator}:${role}`;
        assert.equal(entry.schema, expectedSchemas.get(key), key);
        seenRoles.add(key);
      }
    }
  }
  ...
  assert.deepEqual(
    [...seenRoles].sort(compareCodeUnits),
    [...expectedSchemas.keys()].sort(compareCodeUnits),
  );
});
...
test("covers every required Unit 0 cross-object scenario", () => {
  for (const requiredId of [
    ...
    "manifest.policy-shortening-over-deadline",
    "manifest.seven-day-shortening-at-deadline",
    "manifest.seven-day-shortening-over-deadline",
    "manifest.policy-shortening-unscheduled",
    ...
    "manifest.completed-deletion-identical-revision",
    "manifest.completed-deletion-requested-at-immutable",
    "manifest.completed-deletion-completed-at-immutable",
    "manifest.completed-deletion-object-count-immutable",
    "manifest.completed-deletion-audit-until-immutable",
    "manifest.completed-deletion-event-sequence-immutable",
    "manifest.completed-deletion-extension-immutable",
    "manifest.deleted-retention-effective-policy-immutable",
    "manifest.deleted-retention-content-expiry-immutable",
    "manifest.deleted-retention-updated-at-immutable",
    "manifest.deleted-retention-extension-immutable",
    ...
    "research-run.topic-lineage-conflict",
