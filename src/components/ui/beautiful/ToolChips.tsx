  ...
  return (
    <div className="min-h-[220px] w-full max-w-80 pb-1">
      ...
      <button
        ...
        className="-mx-1.5 flex w-fit items-center gap-1.5 rounded-bui-control px-1.5 py-1 text-[length:var(--text-base)] text-bui-ink-2 transition-colors duration-100 hover:bg-bui-hover-2"
      >
      ...
      <div className="grid transition-[grid-template-rows,opacity] duration-300" style={{ gridTemplateRows: open ? "1fr" : "0fr", opacity: open ? 1 : 0 }}>
        ...
        <div className="mt-1.5 flex flex-col gap-1">
          {ROWS.slice(0, step).map((row) => {
            ...
            return (
            <div key={row.label} className="[animation:bui-fade-up_300ms_cubic-bezier(0.23,1,0.32,1)_both]!">
              <button
                type="button"
                aria-expanded={rowOpen}
                onClick={() => toggleRow(row.label)}
                className="group/row -mx-[3px] flex h-7 w-[calc(100%+6px)] min-w-0 items-center gap-2 rounded-bui-control px-[3px] text-left transition-colors duration-100 hover:bg-bui-hover-2"
              >
                <span className="relative flex size-4 shrink-0 items-center justify-center text-bui-ink-3">
                  <svg
                ...
                <span className="shrink-0 text-[length:var(--text-base)] font-medium text-bui-ink">{row.label}</span>
                <span
                  className={`inline-flex h-5.5 min-w-0 flex-1 cursor-pointer items-center truncate rounded-bui-chip bg-bui-hover-2 px-1.5
                    text-[length:var(--text-sm)] text-bui-ink-2 shadow-bui-hairline transition-colors duration-100 hover:bg-bui-line-strong
                    dark:bg-bui-field dark:hover:bg-bui-hover
                    ${row.mono ? "font-mono" : ""}`}
                >
                  {row.chip}
                </span>
              ...
              >
                <div className="min-h-0 overflow-hidden">
                  <div className="mt-0.5 mb-1 ml-2 flex flex-col gap-0.5 border-l border-bui-line py-0.5 pl-3.5">
                    {row.detail.map((line) => (
                      <span
                        key={line.text}
      ...
      {/* file-diff chips */}
      {step >= total && (
        <div className="mt-2.5 flex max-w-full flex-wrap gap-1.5 border-t border-bui-line pt-2.5">
          {DIFFS.map((d, i) => (
            <span
              key={d.file}
              className="inline-flex h-7 max-w-full cursor-pointer items-center gap-1.5 rounded-bui-chip
                bg-bui-surface px-2 font-mono text-[length:var(--text-sm)] text-bui-ink shadow-bui-btn
                transition-colors duration-100 hover:bg-bui-hover"
          ...
          <button
            type="button"
            className="inline-flex h-7 items-center rounded-bui-chip px-1.5 font-mono text-[length:var(--text-sm)] text-bui-ink-3
              underline decoration-transparent underline-offset-2 transition-colors duration-100
              hover:text-bui-ink-2 hover:decoration-current"
