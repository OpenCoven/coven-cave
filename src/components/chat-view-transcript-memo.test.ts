...
assert.match(
  src,
  /transcriptHandlersRef\.current = \{\s*siblingsFor,\s*switchBranch,\s*editTurnInComposer,\s*regenerateFor,\s*replyFor,\s*askAboutFor,\s*readerPromptFor,\s*rerunWithFor,\s*handleFollowUp,\s*send,\s*\};/,
...
for (const pattern of [
  ...
  /handlers\(\)\.handleFollowUp\(\{ path, sourceText: t\.text \}\)/,
]) {
  assert.match(
    src,
    pattern,
