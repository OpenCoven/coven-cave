...
export function buildCovenMarkersDirective(): string {
  return [
    ...
    'When a decision has a small set of known options, ask it as a card instead of prose: <coven:approve kind="questions" prompt="Which auth approach?" options="Session cookies|JWT bearer|OAuth only" />. Options are pipe-separated (2-6). Write consecutive markers (nothing but a newline between them) to ask up to three questions in one card. A free-text answer is offered automatically; add other="no" when only the listed options are valid. The human\'s answers arrive as their next message, so ask only what you genuinely need to proceed, and never present the choice as already made.',
