...
export function buildAutoModeDirective(mission: string, preferenceDigest?: string | null): string {
  const lines = [
    ...
    "3. The ONLY reasons to interrupt the human again are: (a) the mission is complete, (b) you need their go-ahead on something irreversible — report needs-approval and wait; the mission continues the moment they answer, or (c) you cannot proceed at all — report blocked only when no answer from the human can unblock you (permissions, credentials, something outside your authority that has to change).",
    '4. Report your phase with <coven:auto-status state="clarifying|working|needs-approval|blocked|failed|done" note="short status" /> at each transition — emit it once when you start, again if you need a human, and a final one when the mission ends. Use "needs-approval" when you are stopped on an irreversible step you would take immediately if the human says yes; use "blocked" only when nothing the human types will unblock you. Use "failed" if you hit something you cannot recover from; use "done" only when the mission is actually finished. Keep note short and factual.',
