...
assert.match(
  turnRow,
  /\{!pending && turn\.tools\?\.length && editCards\.length[\s\S]*?\{editedFiles\.length > 1 \? \([\s\S]{0,400}?\{editedFiles\.length\} files changed/,
...
assert.match(
  turnRow,
  /<StreamingTurnResponse[\s\S]*proseContent=\{proseContent\}[\s\S]*activityDetails=\{activityDetails\}[\s\S]*supplementaryContent=\{supplementaryContent\}/,
  "StreamingTurnResponse receives prose, activity, and edit-card supplementary content as separate presentation regions",
);
