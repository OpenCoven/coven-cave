    ...
    <section className="dr-section" aria-label="Needs you">
      ...
      <div className="dr-list">
        ...
        {items.map((item) => {
          ...
          <div
            key={item.id}
            className={[`dr-row dash-inbox__row${selectMode ? " focus-ring-inset" : ""}${selectMode && selectedIds.has(item.id) ? " dash-inbox__row--selected" : ""}`, "[--row-accent:var(--color-warning)]!"].filter(Boolean).join(" ")}
