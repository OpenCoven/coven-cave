...
test("the OpenClaw-only recovery override names the one skipped guard", () => {
  const skipped = render("v7.0.55", undefined, {
    COVEN_RELEASE_OPENCLAW_REGISTRY_GUARD_SKIPPED: "true",
  });
  assert.match(skipped, /^## Build provenance/m);
  assert.match(skipped, /allow_unconfigured_openclaw_registry/);
  assert.match(skipped, /OpenClaw\s+compatibility-registry check did not run/);
  assert.match(skipped, /OpenCode and Grok checks still ran/);
  assert.match(skipped, /tag push cannot skip/);
  assert.doesNotMatch(skipped, /OpenCode, Grok, and OpenClaw compatibility-registry checks did not run/);
  assert.doesNotMatch(
    render("v7.0.55", undefined, {
      COVEN_RELEASE_OPENCLAW_REGISTRY_GUARD_SKIPPED: "1",
    }),
    /allow_unconfigured_openclaw_registry/,
  );
});
