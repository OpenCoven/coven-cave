...
// of NEW chats only, and must never run for resumes, cancels, or ssh runtimes.
{
  ...
  assert.match(
    route,
    /const turnSpawnStartMs = Date\.now\(\);[\s\S]*?if \(hermesNeedsContextReplay\) \{[\s\S]*?await runAttempt\(buildArgs\(null, replay\.prompt\), replay\.prompt\);[\s\S]*?\} else \{\s*await runAttempt\(args, initialAttemptPrompt\);/,
