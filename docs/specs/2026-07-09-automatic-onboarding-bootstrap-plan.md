...
assert.equal(CORE_TOOLS_LOCK.coven.version, "0.2.3");
assert.equal(CORE_TOOLS_LOCK.covenCode.version, "0.6.1");
...
assert.equal(intel.cli.tag, "v0.2.3");
assert.equal(intel.cli.commit, "6804780d0fbbba44b27718d6534aec09a5230218");
...
```json
{
  ...
  "coven": {
    ...
    "intelSource": {
      ...
      "tag": "v0.2.3",
      "tagObject": "053dab8491846bbfa2de61b32dac365728832a8e",
      "commit": "6804780d0fbbba44b27718d6534aec09a5230218"
    }
  },
  "covenCode": {
...
5. Add exact dependencies. Because `@opencoven/coven-code@0.6.1` was published
...
```bash
pnpm add --save-exact @opencoven/cli@0.2.3 @opencoven/coven-code@0.6.1
```
...
await writeFile(path.join(nodeModules, "@opencoven", "cli", "package.json"), '{"version":"0.2.3"}');
await writeFile(path.join(nodeModules, "@opencoven", "coven-code", "package.json"), '{"version":"0.6.1"}');
await writeFile(
...
await stageCoreTools({
  ...
  deps: {
    ...
    probeVersion: async (binary) => binary.endsWith("coven-code") ? "0.6.1" : "0.2.3",
  },
});
...
assert.equal(manifest.tools.coven.version, "0.2.3");
assert.equal(manifest.tools.covenCode.version, "0.6.1");
...
- `licenses/coven-cli-MIT.txt` is the verbatim content of `OpenCoven/coven` `LICENSE` at commit `6804780d0fbbba44b27718d6534aec09a5230218`, Git blob `0cea5dc1a1ce3355c1353b961a8ca7a90367f976`.
- `licenses/coven-code-GPL-3.0.txt` is the verbatim content of `OpenCoven/coven-code` `LICENSE.md` at tag `v0.6.1`, Git blob `871ce8e638ad6d763308e44411d2c4a2e658cf55`.
- `licenses/coven-code-ATTRIBUTION.md` is the verbatim content of `ATTRIBUTION.md` at tag `v0.6.1`, Git blob `74c026f0dc83489fdd7d4ff8d66eb5a81039b783`.
...
await probeVersion(coven, "0.2.3");
await probeVersion(covenCode, "0.6.1");
```
...
assert.deepEqual(status, {
  ...
  source: "bundled",
  target: "linux-x86_64",
  versions: { coven: "0.2.3", covenCode: "0.6.1" },
});
```
...
Expected: generated resource contents are ignored except `placeholder.txt`; verification reports target plus versions 0.2.3 and 0.6.1.
