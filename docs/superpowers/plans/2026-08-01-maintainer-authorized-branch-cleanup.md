...
**Architecture:** Branch Curator selects either the existing automatic profile or a new manual profile before destructive proof begins. The manual profile replaces the unavailable cross-system transaction with current-task authorization, a local maintenance lease, complete fresh fail-closed evidence before every transaction, direct strict `node scripts/worktree-guard.mjs --strict-worktree-remove "$worktree_path" --expected-head "$audited_worktree_head_oid"` allow evidence, and expected-OID mutations; the automatic profile retains the full-gate requirement and never deletes remotes. The existing no-argument Claude hook remains unchanged.
...
- [ ] **Step 1: Reconcile the implementation branch with live `origin/main`**
...
- [ ] **Step 2: Create a section-scoped contract test**
...
test("evals cover every new authorization and race boundary", () => {
  ...
  for (const id of [43, 44, 45, 46, 47, 48, 49, 50]) {
  ...
  assert.match(byId.get(43).expected_output, /manual cleanup profile/i);
  const eval44 = byId.get(44);
  assert.match(eval44.prompt, /current maintainer/i);
  assert.match(eval44.prompt, /exact local-only candidate/i);
  assert.match(eval44.prompt, /current task/i);
  assert.match(eval44.prompt, /both its local and recovery timestamps are older than 24h/);
  assert.match(
    eval44.prompt,
    /configuration-independent inspection finds no staged, unstaged, untracked, ignored, submodule, assume-unchanged, or skip-worktree state/,
  );
  ...
  assert.match(eval44.expected_output, /freshly reverified local cleanup/i);
  assert.match(
    eval44.expected_output,
    /preserves the remote because local authorization does not imply remote deletion/i,
  );
  assert.match(eval44.expected_output, /fresh explicit remote scope/i);
  assert.match(byId.get(45).expected_output, /current task/i);
  assert.match(byId.get(46).prompt, /one untracked file and an active Bead claim/i);
  assert.match(
    byId.get(46).expected_output,
    /dirty path and active Bead claim remain unconditional live-work blockers/i,
  );
  assert.match(byId.get(47).expected_output, /expected OID/i);
  assert.match(byId.get(48).expected_output, /worktree-guard/i);
  assert.match(byId.get(49).expected_output, /protected.*main/i);
  assert.match(byId.get(50).expected_output, /origin fetch\/push destination/i);
  assert.match(byId.get(50).expected_output, /refs\/heads\/feature\/cave-remote:R/);
  assert.match(byId.get(50).expected_output, /:refs\/heads\/feature\/cave-remote/);
  assert.match(byId.get(50).expected_output, /destination or OID drift/i);
  assert.match(byId.get(50).expected_output, /status 2/i);
});
```

- [ ] **Step 3: Wire the test into the API suite**
...
- Modify: `.agents/skills/branch-curator/evals/evals.json:250-257`

- [ ] **Step 1: Make the existing race eval explicitly automatic**
...
- [ ] **Step 2: Replace the remote-recency eval with the manual authority boundary**
...
- [ ] **Step 3: Pin automatic retirement to local-only mutation even under a complete gate**
...
- [ ] **Step 4: Append eight focused evals after eval 42**
...
- [ ] **Step 5: Validate JSON shape and IDs**

Run:

```bash
node -e 'const fs=require("node:fs"); const p=".agents/skills/branch-curator/evals/evals.json"; const x=JSON.parse(fs.readFileSync(p,"utf8")); const ids=x.evals.map(e=>e.id); if(x.evals.length!==50 || new Set(ids).size!==50 || ids.at(-1)!==50) process.exit(1); console.log("branch-curator evals: unique 1..50")'
```

Expected: `branch-curator evals: unique 1..50`.
...
- [ ] **Step 1: Replace the impossible universal gate with explicit profile selection**
...
- [ ] **Step 2: Bind deletion proof to the selected profile**
...
- [ ] **Step 3: Confirm the contract test has advanced to the proof failure**
...
- [ ] **Step 1: Write strict-mode tests and observe RED**

Add tests for this exact direct interface:

```bash
node scripts/worktree-guard.mjs --strict-worktree-remove "$worktree_path" --expected-head "$audited_worktree_head_oid"
```
...
- [ ] **Step 2: Implement the minimal fail-closed strict path**
...
- [ ] **Step 3: Verify strict and legacy guard behavior GREEN**
...
- [ ] **Step 4: Leave normative integration for Task 4**
...
- [x] **Step 1: Run fresh-context skill evals**

Use fresh evaluators for evals 2, 8, 43-50. For each case, provide the repository `AGENTS.md`, the revised Branch Curator skill, its deletion-proof reference, and the single eval prompt. Require the evaluator to return:
...
Observed: ten independent fresh-context evaluators ran evals 2, 8, and 43-50.
All ten returned `pass: true` with the expected profile, authority boundary,
preservation reasons, and exact-proof evidence; no semantic revision was needed.

- [x] **Step 2: Validate JSON and the wired test**

Run:

```bash
node -e 'const fs=require("node:fs"); const x=JSON.parse(fs.readFileSync(".agents/skills/branch-curator/evals/evals.json","utf8")); const ids=x.evals.map(e=>e.id); if(x.evals.length!==50 || new Set(ids).size!==50) process.exit(1); console.log("50 unique evals")'
node --test scripts/branch-curator-manual-cleanup-contract.test.mjs
node scripts/check-tests-wired.mjs
```

Expected: `50 unique evals`; 4 contract tests pass; test wiring check passes.

Observed: `50 unique evals`; the contract passed 4/4; the wiring check reported
all 1436 test files wired with the two documented allowlisted files.

- [ ] **Step 3: Run maintenance and guard tests separately**
...
- [ ] **Step 4: Run repository gates and inspect the exact diff**
...
git diff --check
git status --short --branch
git diff -- .agents/skills/branch-curator AGENTS.md docs/workflows/beads-familiars.md docs/superpowers/specs/2026-07-31-automatic-local-branch-retirement-design.md docs/superpowers/specs/2026-08-01-maintainer-authorized-branch-cleanup-design.md scripts/branch-curator-manual-cleanup-contract.test.mjs scripts/run-tests.mjs
```
...
```bash
bd comment cave-3hpv8 "Implemented the approved manual cleanup profile in fix/cave-3hpv8-maintainer-cleanup. Verification: 50 unique eval IDs; focused fresh-context evals matched their expected profiles and safety outcomes; branch-curator contract 4/4; check-tests-wired passed; maintenance-gate passed; worktree-guard passed; pnpm lint passed; pnpm beads:worktrees completed; git diff --check clean."
```
...
SSH_AUTH_SOCK="$agent_socket" git add AGENTS.md .agents/skills/branch-curator/SKILL.md .agents/skills/branch-curator/references/deletion-proof.md .agents/skills/branch-curator/evals/evals.json docs/workflows/beads-familiars.md docs/superpowers/specs/2026-07-31-automatic-local-branch-retirement-design.md docs/superpowers/specs/2026-08-01-maintainer-authorized-branch-cleanup-design.md docs/superpowers/plans/2026-08-01-maintainer-authorized-branch-cleanup.md scripts/branch-curator-manual-cleanup-contract.test.mjs scripts/run-tests.mjs
...
- 50 unique Branch Curator evals
