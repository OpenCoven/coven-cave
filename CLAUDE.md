...
- Required status checks (all must pass) — **five**, in both layers: `Frontend build`, `Rust check`, `E2E (Playwright)`, `Cross-environment required`, `Sidecar runtime required`. The two layers are classic branch protection and ruleset `19123333` ("main protection (checks)"); they list the same five, so a check added to one and not the other is drift.
  **`CodeQL` is no longer required** (removed from both layers 2026-07-31, cave-3veeb; the ruleset was renamed from "main protection (checks + CodeQL)" to drop it). Do not re-add it on the strength of an older note — this file said the opposite until 2026-07-31. If it is ever re-required, two things bit before and will bite again: require the **aggregate** `CodeQL` context and never the per-language `Analyze (<lang>)` jobs (those match ambiguously and hang as "expected"), and know that the aggregate stopped attaching to new PR heads in July 2026 — a required context that never reports leaves a PR `BLOCKED` with every check green and no failing job to fix.
  The `E2E (Playwright)` job runs daemon-less (`COVEN_CAVE_E2E=1`), so e2e specs must be self-contained — dismiss onboarding (`cave:onboarding:dismissed=1`) and drive surfaces via `page.route(...)` API mocks rather than a live daemon.
