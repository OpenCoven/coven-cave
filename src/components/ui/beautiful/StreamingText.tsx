  ...
  return (
    <a
      ...
      className="ml-0 mr-1 inline-flex h-4.5 translate-y-[-1px] items-center gap-1 rounded-[5px]
        bg-bui-inset pr-[3px] pl-[3px] align-middle font-mono text-[length:var(--text-xs)] text-bui-ink-2 shadow-bui-hairline
        transition-colors duration-150 hover:bg-bui-hover hover:text-bui-ink [animation:bui-pop-in_250ms_cubic-bezier(0.23,1,0.32,1)_both]!"
    >
    ...
    <div className="min-h-[15.5rem] w-full max-w-95">
      <p className="text-[length:var(--text-base)] leading-relaxed text-bui-ink">
        {TOKENS.slice(0, count).map((token, i) =>
          token.cite ? (
            ...
            <span
              key={i}
              className="inline [will-change:filter,opacity] [animation:bui-stream-in_420ms_cubic-bezier(0.22,0.61,0.25,1)_both]!"
            >
              {token.text}{" "}
            </span>
          ),
        ...
        {!done && (
          <span
            className="ml-0.5 inline-block h-3 w-0.5 translate-y-0.5 rounded-full bg-bui-ink [animation:bui-fade-in_150ms_ease-out_both]!"
          />
        )}
      </p>
      ...
      >
        {ACTION_ICONS.map((icon, i) => (
          <button
            ...
            className="flex size-6 items-center justify-center rounded-[6px] text-bui-ink-3
              transition-colors duration-100 hover:bg-bui-hover-2 hover:text-bui-ink-2"
          >
        ...
        <button
          ...
          className="ml-1.5 flex items-center gap-1.5 rounded-[6px] px-1 py-0.5 text-left transition-colors duration-150 hover:bg-bui-hover"
        >
          <span className="flex -space-x-1">
            {SOURCES.map((source) => (
              <img
                ...
                alt=""
                className="source-avatar size-3.5 rounded-full bg-bui-surface shadow-[0_0_0_1.5px_var(--bui-canvas)]"
              />
            ))}
          </span>
          <span className="text-[length:var(--text-sm)] text-bui-ink-2">10 sources</span>
        </button>
      </div>
      ...
      >
        <div className="overflow-hidden">
          <div className="mt-1.5 flex flex-col rounded-[10px] bg-bui-inset p-1 shadow-bui-hairline">
            {SOURCES.map((source) => (
              <a
                key={source.domain}
                href={source.href}
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2 rounded-[6px] px-1.5 py-1 text-[length:var(--text-sm)] text-bui-ink-2 transition-colors duration-150 hover:bg-bui-hover hover:text-bui-ink"
              >
                ...
                <span className="ml-auto font-mono text-[length:var(--text-xs)] text-bui-ink-3">{source.domain}</span>
              </a>
            ))}
          </div>
        </div>
      ...
      <div
        ...
        style={{ opacity: done ? 1 : 0, pointerEvents: done ? "auto" : "none" }}
      >
        <p className="text-[length:var(--text-sm)] font-medium text-bui-ink-2">Follow-ups</p>
        <div className="mt-0.5 flex flex-col">
          {FOLLOW_UPS.map((text, i) => (
            <button
              key={text}
              className="-mx-1.5 flex items-center gap-2 rounded-[7px] border-b border-bui-line
                px-1.5 py-1.5 text-left text-[length:var(--text-base)] text-bui-ink transition-colors
                duration-100 hover:bg-bui-hover-2"
              style={
                done
            ...
            >
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--bui-ink-3)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
