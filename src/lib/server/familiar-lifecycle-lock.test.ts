...
const workdir = await mkdtemp(path.join(testTmpRoot, "familiar-lifecycle-lock-"));
...
const {
  familiarLifecycleLockDbPath,
  withFamiliarLifecycleLock,
} = await import("./familiar-lifecycle-lock.ts");
...
test("familiarLifecycleLockDbPath derives a sidecar path adjacent to the config store file", () => {
  const store = storePathFor("derivation-check");
  assert.equal(familiarLifecycleLockDbPath(store), `${store}.lifecycle-lock.sqlite3`);
});

// ─── in-process mutual exclusion ──────────────────────────────────────────

test("two concurrent in-process lifecycle critical sections on the same store path never overlap", async () => {
  ...
  async function critical(id: string): Promise<void> {
    await withFamiliarLifecycleLock({ storePath }, async () => {
      inside += 1;
  ...
  assert.equal(maxObservedInside, 1, "at most one lifecycle critical section may run at once");
...
test("an operation that rejects still releases the lock for the next caller", async () => {
  ...
  await assert.rejects(
    withFamiliarLifecycleLock({ storePath }, async () => {
      throw new Error("simulated lifecycle effect failure");
    }),
    /simulated lifecycle effect failure/,
  );

  const result = await withFamiliarLifecycleLock({ storePath }, async () => "lock-is-free");
...
test("a caller that times out waiting for a held lock does not poison it for the eventual next caller", async () => {
  ...
  const holderEntered = new Promise<void>((resolve) => {
    void withFamiliarLifecycleLock({ storePath }, async () => {
      resolve();
  ...
  await assert.rejects(
    withFamiliarLifecycleLock({ storePath, timeoutMs: 150 }, async () => "should not run"),
    /timed out/,
    "a second caller must give up rather than wait forever",
  );

  holderRelease.release?.();
  const result = await withFamiliarLifecycleLock({ storePath }, async () => "lock-is-free-again");
...
test("a real subprocess holding the lock blocks this process's acquisition until it releases", async () => {
  ...
  const moduleUrl = pathToFileURL(
    path.resolve("src/lib/server/familiar-lifecycle-lock.ts"),
  ).href;
  const holdMs = 400;
  const holderScript = `
    const { withFamiliarLifecycleLock } = await import(${JSON.stringify(moduleUrl)});
    await withFamiliarLifecycleLock({ storePath: ${JSON.stringify(storePath)} }, async () => {
  ...
  const result = await withFamiliarLifecycleLock({ storePath }, async () => "acquired-after-release");
...
test("SIGKILLing a subprocess mid-lifecycle-effect releases the lock almost immediately, not after the timeout", async () => {
  ...
  const startedAt = Date.now();
  const result = await withFamiliarLifecycleLock(
...
test("the lock database file is created adjacent to the config store path and is not JSON", async () => {
  const storePath = storePathFor("lock-file-shape");
  await withFamiliarLifecycleLock({ storePath }, async () => "done");

  const lockDbPath = familiarLifecycleLockDbPath(storePath);
...
test("two subprocesses racing withFamiliarLifecycleLock against the same store path both eventually run, and neither clobbers the other's read-modify-write", async () => {
