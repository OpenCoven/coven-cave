...
6. From the plugin root, run `node bin/tweet-thread-validate.mjs validate <candidate.json> [brief.json]` for every candidate. Exit `0` is accepted, exit `1` is a deterministic hard-gate rejection, and exit `2` is a usage/read/parse/runtime contract error. Save the strict JSON stdout as the raw deterministic result, then materialize the canonical `validation-` record with a caller-supplied stable ID and timestamp. Materialize the canonical validation record before the manifest or approval. Only after the bundled command succeeds may `cave.output` record the raw result and canonical validation artifact. Append rejections, contract errors, partial materialization, missing paths, uncertainty, and stopping decisions to `execution-log.jsonl`.
...
- Write `brief.json`, `evidence.json`, `voice-profile.json`, `strategies.json`, `execution-log.jsonl`, candidate files, raw deterministic result files, canonical validation records, judge scorecards, `manifest.json`, and, after a human decision, `approval.json`.
...
- Confirm every approved or publishing candidate has exactly one accepted validation record, no `fail` validation finding, and at least one bound scorecard with no `fail` finding.
