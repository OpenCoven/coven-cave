...
const cacheSource = readFileSync(
  new URL("../../../../lib/server/sessions-list-cache.ts", import.meta.url),
  "utf8",
);
...
assert.match(
  ...
  "the route delegates collapse-aware reads through the shared cache",
);
...
assert.equal(
  (helperSource.match(/applyFamiliarWorkspaceCollapse\(/g) || []).length,
  3,
...
assert.match(
  helperSource,
  /if \(hasActiveChatRun\(conv\.sessionId\)\) \{\s*return \{ \.\.\.conv, status: "running", exitCode: 0 \};\s*\}\s*if \(conv\.pending\) return \{ \.\.\.conv, status: "failed", exitCode: 1 \};\s*\n\s*return conv;/,
