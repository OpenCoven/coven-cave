...
import { isVaultKeyGrantedTo, loadVaultMap, normalizeVaultScope, saveVaultMap } from "../grimoire/vault.ts";
...
try {
  const { loadVaultMap, saveVaultMap } = await import("../grimoire/vault.ts");
  saveVaultMap({
    SHARED_TOKEN: { ref: "op://a/b/c" },
...
const daemonStartSource = read("../daemon/daemon-start.ts");
assert.match(
  daemonStartSource,
