...
// globally and each full-window title band reserves the lights inset.
{
  ...
  assert.match(
    settingsShell,
    // The attribute became conditional — an embedded settings pane correctly
    // drops the drag region — so pin that "deep" is what a non-embedded header
    // gets, not that the attribute is a bare literal (cave-ktvy0).
