...
assert.match(
  source,
  /const CHAT_ATTACHMENT_ACCEPT = \[[\s\S]*"image\/\*"[\s\S]*"video\/\*"[\s\S]*"audio\/\*"[\s\S]*"application\/pdf"[\s\S]*"application\/zip"[\s\S]*"\.md"[\s\S]*"\.json"[\s\S]*"\.zip"[\s\S]*\]\.join\(","\)/,
  "Chat attachments should explicitly accept images, videos, audio, documents, archives, and common text/code files",
);
